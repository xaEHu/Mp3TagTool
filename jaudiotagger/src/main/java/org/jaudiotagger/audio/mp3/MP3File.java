...
public class MP3File extends AudioFile
{
    ...
    private void readV1Tag(Context context,Uri uri, int loadOptions) throws IOException, CannotReadException
    {
        if ((loadOptions & LOAD_IDV1TAG) != 0)
        {
            logger.finer("Attempting to read id3v1tags");
            try
            {
                id3v1tag = new ID3v11Tag(context,uri, uri.getPath());
            }
            catch (TagNotFoundException ex)
            {
                logger.config("No ids3v11 tag found");
            ...
            try
            {
                if (id3v1tag == null)
                {
                    id3v1tag = new ID3v1Tag(context,uri, uri.getPath());
                }
            }
            catch (TagNotFoundException ex)
            {
        ...
        if (startByte >= AbstractID3v2Tag.TAG_HEADER_LENGTH)
        {
            ...
            try
            {
                ...
                if ((loadOptions & LOAD_IDV2TAG) != 0)
                {
                    ...
                    try
                    {
                        this.setID3v2Tag(new ID3v24Tag(bb, name));
                    }
                    catch (TagNotFoundException ex)
                    {
                    ...
                    try
                    {
                        if (id3v2tag == null)
                        {
                            this.setID3v2Tag(new ID3v23Tag(bb, name));
                        }
                    }
                    catch (TagNotFoundException ex)
                    {
                    ...
                    try
                    {
                        if (id3v2tag == null)
                        {
                            this.setID3v2Tag(new ID3v22Tag(bb, name));
                        }
                    }
                    catch (TagNotFoundException ex)
                    {
    ...
    private void readV2Tag(Context context, Uri uri, int loadOptions, int startByte) throws IOException, TagException, CannotReadException
    {
        //We know where the actual Audio starts so load all the file from start to that point into
        //a buffer then we can read the IDv2 information without needing any more File I/O
        ...
        if (startByte >= AbstractID3v2Tag.TAG_HEADER_LENGTH)
        {
            ...
            try
            {
                fis = ContextUtils.getParcelInputStream(context,uri);
                final ByteBuffer bb = ByteBuffer.allocateDirect(startByte);
                fis.getChannel().read(bb,0);
                bb.rewind();
    ...
    public MP3File(Context context, Uri uri, int loadOptions, boolean readOnly) throws IOException, TagException, ReadOnlyFileException, CannotReadException, InvalidAudioFrameException
    {
...
//            newFile = checkFilePermissions(file, readOnly);
            ...
            //Read ID3v2 tag size (if tag exists) to allow audioHeader parsing to skip over tag
            long tagSizeReportedByHeader = AbstractID3v2Tag.getV2TagSizeIfExists(context, uri);
            logger.config("TagHeaderSize:" + Hex.asHex(tagSizeReportedByHeader));
            audioHeader = new MP3AudioHeader(context,uri, tagSizeReportedByHeader);
            ...
            //Read v1 tags (if any)
            readV1Tag(context, uri, loadOptions);

            //Read v2 tags (if any)
            readV2Tag(context,uri, loadOptions, (int)((MP3AudioHeader) audioHeader).getMp3StartByte());
