...
public class OggInfoReader
{
    ...
    public GenericAudioHeader read(Context context, Uri uri) throws CannotReadException, IOException
    {
        ...
        //Check start of file does it have Ogg pattern
        byte[] b = new byte[OggPageHeader.CAPTURE_PATTERN.length];
        ByteBuffer byteBuffers = ByteBuffer.wrap(b);
        channel.read(byteBuffers);
        if (!(Arrays.equals(b, OggPageHeader.CAPTURE_PATTERN)))
        {
            channel.position(0);
            if(AbstractID3v2Tag.isId3Tag(channel))
            {
                channel.read(byteBuffers);
                if ((Arrays.equals(b, OggPageHeader.CAPTURE_PATTERN)))
                {
        ...
        while (channel.position() >= 4)
        {
            if (channel.read(ByteBuffer.allocate(1)) == OggPageHeader.CAPTURE_PATTERN[3])
            {
                channel.position(channel.position() - OggPageHeader.FIELD_CAPTURE_PATTERN_LENGTH);
                byte[] ogg = new byte[3];
                channel.read(ByteBuffer.wrap(ogg));
                if (ogg[0] == OggPageHeader.CAPTURE_PATTERN[0] && ogg[1] == OggPageHeader.CAPTURE_PATTERN[1] && ogg[2] == OggPageHeader.CAPTURE_PATTERN[2])
                {
                    channel.position( channel.position() - 3);

                    oldPos =  channel.position();
                    channel.position( channel.position() + OggPageHeader.FIELD_PAGE_SEGMENTS_POS);
                    int pageSegments = channel.read(ByteBuffer.allocate(1)) & 0xFF; //Unsigned
        ...
        //1st page = Identification Header
        OggPageHeader pageHeader = OggPageHeader.read(channel);
        byte[] vorbisData = new byte[pageHeader.getPageLength()];
        ByteBuffer vorbisDataWrap = ByteBuffer.wrap(vorbisData);

        if(vorbisData.length < OggPageHeader.OGG_PAGE_HEADER_FIXED_LENGTH)
        {
            throw new CannotReadException("Invalid Identification header for this Ogg File");
        }
        channel.read(vorbisDataWrap);
        VorbisIdentificationHeader vorbisIdentificationHeader = new VorbisIdentificationHeader(vorbisData);
        ...
        {
            //TODO need to remove comment from raf.getLength()
            info.setBitRate(computeBitrate(info.getTrackLength(), channel.size()));
            info.setVariableBitRate(true);
        }
        return info;
    }
