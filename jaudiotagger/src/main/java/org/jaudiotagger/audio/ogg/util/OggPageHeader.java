...
public class OggPageHeader
{
    ...
    public static OggPageHeader read(FileChannel fileChannel) throws IOException, CannotReadException
    {
        long start = 0;
        logger.fine("Trying to read OggPage at:" + start);
        ...
        if (!(Arrays.equals(b, OggPageHeader.CAPTURE_PATTERN)))
        {
            ...
            {
                logger.warning(ErrorMessage.OGG_CONTAINS_ID3TAG.getMsg(fileChannel.position() - start));
                fileChannel.read(ByteBuffer.wrap(b));
                if ((Arrays.equals(b, OggPageHeader.CAPTURE_PATTERN)))
                {
                    //Go to the end of the ID3 header
                    start=fileChannel.position() - OggPageHeader.CAPTURE_PATTERN.length;
                }
            }
            else
            {
        ...
        fileChannel.position(start + OggPageHeader.FIELD_PAGE_SEGMENTS_POS);
        int pageSegments = fileChannel.read(ByteBuffer.allocate(1)) & 0xFF; //unsigned
        fileChannel.position(start);
