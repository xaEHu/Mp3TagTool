...
public class OggVorbisTagReader
{
    ...
    public Tag read(Context context, Uri uri) throws CannotReadException, IOException
    {
        logger.config("Starting to read ogg vorbis tag from file:");
        byte[] rawVorbisCommentData = readRawPacketData(context, uri);
    ...
    public byte[] readRawPacketData(Context context, Uri uri) throws CannotReadException, IOException
    {
        ...
        //Skip over data to end of page header 1
        channel.position(channel.position() + pageHeader.getPageLength());
//        raf.seek(raf.getFilePointer() + pageHeader.getPageLength());

        logger.fine("Read 2nd page");
        //2nd page = comment, may extend to additional pages or not , may also have setup header
        pageHeader = OggPageHeader.read(channel);
        ...
        //Convert the comment raw data which maybe over many pages back into raw packet
        byte[] rawVorbisCommentData = convertToVorbisCommentPacket(pageHeader, channel);
        return rawVorbisCommentData;
    }
    ...
    private byte[] convertToVorbisCommentPacket(OggPageHeader startVorbisCommentPage, FileChannel channel) throws IOException, CannotReadException
    {
        ByteArrayOutputStream baos = new ByteArrayOutputStream();
        byte[] b = new byte[startVorbisCommentPage.getPacketList().get(0).getLength() - (VorbisHeader.FIELD_PACKET_TYPE_LENGTH + VorbisHeader.FIELD_CAPTURE_PATTERN_LENGTH)];
        channel.read(ByteBuffer.wrap(b));
        ...
        {
            logger.config("Reading next page");
            OggPageHeader nextPageHeader = OggPageHeader.read(channel);
            b = new byte[nextPageHeader.getPacketList().get(0).getLength()];
            channel.read(ByteBuffer.wrap(b));
            baos.write(b);
