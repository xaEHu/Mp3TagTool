...
public class RealFileReader extends AudioFileReader
{
    ...
    protected GenericAudioHeader getEncodingInfo(Context context, Uri uri) throws CannotReadException, IOException {
        FileChannel fileChannel = ContextUtils.getParcelInputStream(context, uri).getChannel();
        final GenericAudioHeader rv = new GenericAudioHeader();
        final RealChunk prop = findPropChunk(fileChannel);
        final DataInputStream dis = prop.getDataInputStream();
        final int objVersion = Utils.readUint16(dis);
        if (objVersion == 0)
        {
    ...
    private RealChunk findPropChunk(FileChannel fileChannel) throws IOException, CannotReadException
    {
        final RealChunk rmf = RealChunk.readChunk(fileChannel);
        final RealChunk prop = RealChunk.readChunk(fileChannel);
        return prop;
    }
    ...
    private RealChunk findContChunk(FileChannel fileChannel) throws IOException, CannotReadException
    {
        final RealChunk rmf = RealChunk.readChunk(fileChannel);
        final RealChunk prop = RealChunk.readChunk(fileChannel);
        RealChunk rv = RealChunk.readChunk(fileChannel);
        while (!rv.isCONT()) rv = RealChunk.readChunk(fileChannel);
        return rv;
    }
    ...
    protected Tag getTag(Context context, Uri uri) throws CannotReadException, IOException {
        FileChannel fileChannel = ContextUtils.getParcelInputStream(context, uri).getChannel();
        final RealChunk cont = findContChunk(fileChannel);
        final DataInputStream dis = cont.getDataInputStream();
        final String title = Utils.readString(dis, Utils.readUint16(dis));
        final String author = Utils.readString(dis, Utils.readUint16(dis));
        final String copyright = Utils.readString(dis, Utils.readUint16(dis));
