...
public class RealChunk {
	...
	public static RealChunk readChunk(FileChannel fileChannel)
			throws CannotReadException, IOException {
		final String id = Utils.readString(fileChannel, 4);
		final int size = (int)Utils.readUint32(fileChannel);
		if (size < 8) {
			throw new CannotReadException(
					"Corrupt file: RealAudio chunk length at position "
		...
		if (size > (fileChannel.size() - fileChannel.position() + 8)) {
			throw new CannotReadException(
					"Corrupt file: RealAudio chunk length of " + size
							+ " at position " + (fileChannel.position() - 4)
							+ " extends beyond the end of the file");
		}
		final byte[] bytes = new byte[size - 8];
