...
public class AiffInfoReader extends AiffChunkReader {
    ...
    protected GenericAudioHeader read(Context context, Uri uri) throws CannotReadException, IOException {
        FileInputStream inputStream = null;
        try {
            inputStream = ContextUtils.getParcelInputStream(context, uri);
            FileChannel fc = inputStream.getChannel();
            logger.config(uri.getPath() + " Reading AIFF file size:" + Hex.asDecAndHex(fc.size()));
            AiffAudioHeader aiffAudioHeader = new AiffAudioHeader();
            final AiffFileHeader fileHeader = new AiffFileHeader();
            long noOfBytes = fileHeader.readHeader(fc, aiffAudioHeader, uri.getPath());
            while (fc.position() < fc.size()) {
                if (!readChunk(fc, aiffAudioHeader, uri.getPath())) {
                    logger.severe(uri.getPath() + " UnableToReadProcessChunk");
                    break;
                }
            }
            calculateBitRate(aiffAudioHeader);
            ...
            AudioFileIO.closeQuietly(inputStream);
        }
    }
