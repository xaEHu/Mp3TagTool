...
public class AiffTagReader extends AiffChunkReader {
    ...
    public AiffTag read(Context context, Uri uri) throws CannotReadException, IOException {
        ...
        try {
            ...
            final AiffFileHeader fileHeader = new AiffFileHeader();
            fileHeader.readHeader(fc, aiffAudioHeader, uri.getPath());
            while (fc.position() < fc.size()) {
                if (!readChunk(fc, aiffTag, uri.getPath())) {
