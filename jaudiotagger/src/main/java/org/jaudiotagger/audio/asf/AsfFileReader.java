...
import org.jaudiotagger.ContextUtils;
...
public class AsfFileReader extends AudioFileReader
{
    ...
    protected GenericAudioHeader getEncodingInfo(Context context, Uri uri) throws CannotReadException, IOException {
        FileInputStream inputStream = ContextUtils.getParcelInputStream(context, uri);
        inputStream.getChannel().position(0);
        GenericAudioHeader info;
        try
        {
            final AsfHeader header = AsfHeaderReader.readInfoHeader(inputStream);
            if (header == null)
            {
                throw new CannotReadException("Some values must have been " + "incorrect for interpretation as asf with wma content.");
            }
    ...
    protected Tag getTag(Context context, Uri uri) throws CannotReadException, IOException {
        ...
        try
        {
            final AsfHeader header = AsfHeaderReader.readTagHeader(inputStream);
            if (header == null)
            {
                throw new CannotReadException("Some values must have been " + "incorrect for interpretation as asf with wma content.");
            }
...
//        }
        ...
        try
        {
            FileInputStream fileInputStream = ContextUtils.getParcelInputStream(context, uri);
            stream = new FullRequestInputStream(new BufferedInputStream(fileInputStream));
            final AsfHeader header = HEADER_READER.read(Utils.readGUID(stream), stream, 0);
            if (header == null)
            {
                throw new CannotReadException(ErrorMessage.ASF_HEADER_MISSING.getMsg(uri.getPath()));
            }
            if (header.getFileHeader() == null)
            {
                throw new CannotReadException(ErrorMessage.ASF_FILE_HEADER_MISSING.getMsg(uri.getPath()));
            }

            // Just log a warning because file seems to play okay
            if (header.getFileHeader().getFileSize().longValue() != fileInputStream.getChannel().size())
            {
                logger.warning(ErrorMessage.ASF_FILE_HEADER_SIZE_DOES_NOT_MATCH_FILE_SIZE.getMsg(uri.getPath(), header.getFileHeader().getFileSize().longValue(), fileInputStream.getChannel().size()));
            }

            return new AudioFile(uri, getAudioHeader(header), getTag(header));
        ...
        {
            throw new CannotReadException("\"" + uri.getPath() + "\" :" + e, e);
        }
        finally
        {
            try
