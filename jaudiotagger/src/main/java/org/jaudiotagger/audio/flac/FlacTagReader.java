...
public class FlacTagReader {
    ...
    public FlacTag read(Context context, Uri uri) throws CannotReadException, IOException {
        ...
        try {
            ...
            while (!isLastBlock) {
                if (logger.isLoggable(Level.CONFIG)) {
                    logger.config(uri.getPath() + " Looking for MetaBlockHeader at:" + fc.position());
                }
                ...
                if (logger.isLoggable(Level.CONFIG)) {
                    logger.config(uri.getPath() + " Reading MetadataBlockHeader:" + mbh.toString() + " ending at " + fc.position());
                }
                    ...
                    switch (mbh.getBlockType()) {
                        ...
                        case PICTURE:
                            try {
                                ...
                                logger.warning(uri.getPath() + "Unable to read picture metablock, ignoring:" + ioe.getMessage());
                            } catch (InvalidFrameException ive) {
                                logger.warning(uri.getPath() + "Unable to read picture metablock, ignoring" + ive.getMessage());
                            }
                        ...
                        default:
                            if (logger.isLoggable(Level.CONFIG)) {
                                logger.config(uri.getPath() + "Ignoring MetadataBlock:" + mbh.getBlockType());
                            }
                            fc.position(fc.position() + mbh.getDataLength());
                            break;
                    }
            ...
            AudioFileIO.closeQuietly(fileDescriptor);
        }
    }
}
