...
public class FlacInfoReader {
    ...
    public FlacAudioHeader read(Context context, Uri uri) throws CannotReadException, IOException {
        ...
        try {
            ...
            while (isLastBlock == false) {
                MetadataBlockHeader mbh = MetadataBlockHeader.readHeader(fc);
                logger.info(uri.getPath() + " " + mbh.toString());
                if (mbh.getBlockType() == BlockType.STREAMINFO) {
                    mbdsi = new MetadataBlockDataStreamInfo(mbh, fc);
                    if (!mbdsi.isValid()) {
                        throw new CannotReadException(uri.getPath() + ":FLAC StreamInfo not valid");
                    }
                } else {
                    fc.position(fc.position() + mbh.getDataLength());
                }
            ...
            if (mbdsi == null) {
                throw new CannotReadException(uri.getPath() + ":Unable to find Flac StreamInfo");
            }
