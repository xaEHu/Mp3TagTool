...
public class DsfFileReader extends AudioFileReader2 {
    ...
    protected GenericAudioHeader getEncodingInfo(Context context, Uri uri) throws CannotReadException, IOException {
        ...
        try {
            fileInpuStream = ContextUtils.getParcelInputStream(context, uri);
            FileChannel fc = fileInpuStream.getChannel();
            DsdChunk dsd = DsdChunk.readChunk(Utils.readFileDataIntoBufferLE(fc, DsdChunk.DSD_HEADER_LENGTH));
            if (dsd != null) {
                ByteBuffer fmtChunkBuffer = Utils.readFileDataIntoBufferLE(fc, IffHeaderChunk.SIGNATURE_LENGTH + CHUNKSIZE_LENGTH);
                FmtChunk fmt = FmtChunk.readChunkHeader(fmtChunkBuffer);
                if (fmt != null) {
                    ...
                    throw new CannotReadException(uri.getPath() + " Not a valid dsf file. Content does not include 'fmt ' chunk");
                }
            } else {
                throw new CannotReadException(uri.getPath() + " Not a valid dsf file. Content does not start with 'DSD '");
            }
        } finally {
            AudioFileIO.closeQuietly(fileInpuStream);
        }
    }
    ...
    protected Tag getTag(Context context, Uri uri) throws CannotReadException, IOException {
        ...
        try {
            fileInputStream = ContextUtils.getParcelInputStream(context, uri);
            FileChannel fc = fileInputStream.getChannel();
            DsdChunk dsd = DsdChunk.readChunk(Utils.readFileDataIntoBufferLE(fc, DsdChunk.DSD_HEADER_LENGTH));
            if (dsd != null) {
                return readTag(fc, dsd, uri.getPath());
            } else {
                throw new CannotReadException(uri.getPath() + " Not a valid dsf file. Content does not start with 'DSD '.");
            }
        } finally {
