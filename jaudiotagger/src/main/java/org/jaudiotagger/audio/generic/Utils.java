...
public class Utils {
    ...
    public static long readUint32(final FileChannel fileChannel) throws IOException {
        final byte[] buf8 = {0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00};
        ByteBuffer[] byteBuffers = new ByteBuffer[buf8.length];
        for (int i = 0; i < buf8.length; i++) {
            byteBuffers[i] = ByteBuffer.wrap(buf8,i,1);
        }
        fileChannel.read(byteBuffers, 4, 4);
        return ByteBuffer.wrap(buf8).getLong();
    }
    ...
    public static String readString(final FileChannel fileChannel, final int charsToRead) throws IOException {
        final byte[] buf = new byte[charsToRead];
        fileChannel.read(ByteBuffer.wrap(buf));
        return new String(buf, US_ASCII);
    }
    /**
