...
public abstract class AudioFileReader
{
    ...
    */
    ...
    protected abstract GenericAudioHeader getEncodingInfo(Context context, Uri uri) throws CannotReadException, IOException;
    ...
    protected abstract Tag getTag(Context context, Uri uri) throws  CannotReadException, IOException;
    ...
    public AudioFile read(Context context, Uri uri) throws CannotReadException, IOException, TagException, ReadOnlyFileException, InvalidAudioFrameException
    {
        if(logger.isLoggable(Level.CONFIG))
        {
            logger.config(ErrorMessage.GENERAL_READ.getMsg(uri.getPath()));
        }

        ParcelFileDescriptor fileDescriptor = context.getContentResolver().openFileDescriptor(uri,"rw");

//        if (!f.canRead())
//        {
//            throw new NoReadPermissionsException(ErrorMessage.GENERAL_READ_FAILED_DO_NOT_HAVE_PERMISSION_TO_READ_FILE.getMsg(uri.getPath()));
//        }

        FileChannel channel = new ParcelFileDescriptor.AutoCloseInputStream(fileDescriptor).getChannel();

        if (channel.size() <= MINIMUM_SIZE_FOR_VALID_AUDIO_FILE)
        {
            throw new CannotReadException(ErrorMessage.GENERAL_READ_FAILED_FILE_TOO_SMALL.getMsg(uri.getPath()));
        }
...
//            raf = new RandomAccessFile(f, "r");
            ...
            GenericAudioHeader info = getEncodingInfo(context,uri);
            channel.position(0);
            Tag tag = getTag(context,uri);
            return new AudioFile(uri, info, tag);
        ...
        {
            logger.log(Level.SEVERE, ErrorMessage.GENERAL_READ.getMsg(uri.getPath()),e);
            throw new CannotReadException(uri.getPath()+":" + e.getMessage(), e);
        }
        finally
        {
            try
            ...
            {
                logger.log(Level.WARNING, ErrorMessage.GENERAL_READ_FAILED_UNABLE_TO_CLOSE_RANDOM_ACCESS_FILE.getMsg(uri.getPath()));
            }
        }
    }
}
