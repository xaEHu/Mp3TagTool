...
public class Mp4AtomTree
{
    ...
    public Mp4AtomTree(FileChannel channel, boolean closeOnExit) throws IOException, CannotReadException
    {
        buildTree(channel, closeOnExit);
    }
    /**
    ...
    {
        return buildTree(raf.getChannel(),closeExit);
    }

    public DefaultTreeModel buildTree(Context context, Uri uri, boolean closeExit) throws IOException, CannotReadException
    {
        return buildTree(ContextUtils.getParcelInputStream(context, uri).getChannel(),closeExit);
    }
